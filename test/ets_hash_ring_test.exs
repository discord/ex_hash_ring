defmodule ETSHashRingTest do
  ...
  test "hash ring" do
    for num_replicas <- replicas() do
      {:ok, pid} = HashRing.ETS.start_link(ETSHashRingTest, nodes(), num_replicas)
      for key <- keys() do
        assert HashRing.ETS.find_node(ETSHashRingTest, key) == find_node(num_replicas, key)
        assert HashRing.ETS.find_nodes(ETSHashRingTest, key, num()) == find_nodes(num_replicas, key, num())
        :ok
      end
      GenServer.stop(pid)
    end
